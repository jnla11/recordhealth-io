...
Status: historical, design v1.3 (shape, not spec) — superseded 2026-09-18 by v1.4 (`ADI_GRADING_DESIGN_v1.4.md`), which carries v1.3 forward and applies the owner rulings of 2026-09-17/18 (GR-28 onward). Read v1.4 for the current design; this file is kept for history.
...
4. **The tree (console) — SHIPPED 2026-09-17.** Sections > atoms; the tree filters; GR-11's filters; moves write `member_of_section` / `section_in_section`; the list-only hold retires. Close: one atom re-homed through the tree and carried in the corrected core.
...
- GR-24 (2026-09-17): moves are written through the relationship route only; created atoms are valid endpoints; a section is never moved under its own descendant (console refusal).
- GR-25 (2026-09-17): a move-back withdraws the reviewer's own reject and writes no accepted; the withdrawal counts as a move at its own log position.
...
- GR-27 (2026-09-17): the §5 retirement and open-axes ruling (§5).
- Build step 4 (§10 item 4) shipped 2026-09-17 in `recordhealth-api`, commits `d919abb..3df7042`, `rh.rebuild/4`; see that repo's `docs/archive/SESSION_LOG.md`.
