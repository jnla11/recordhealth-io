...
**Generations stay per-phase.** Parse and extract are genuinely concurrent arms; a unified job-level generation would force serialization. What unifies is the mechanism: one claimLease/renewLease/completeLease/expireLease applier family replaces the four hand-built variants. Per-phase watchdog keys collapse into one `leases[phase] = {gen, step, deadline}` shape. Attempt counting unifies into the recovery-epoch ceiling (§B), so layers can no longer multiply.
...
- **F-NEW-MX** plugs in after rung 3 (needs the ledger rollup and the hold machinery); its plug points (§G) exist from rung 3b and default to proceed until MX ships.
