# INGEST_LIVENESS_DESIGN.md — Ingest Liveness, Spend, and Observability Design (v7)

Status: DRAFT v7
Last verified: 2026-08-26

**Provenance:** produced across two Fable design sessions on 2026-07-29; revised against audits of F-NEW-MR and the duplicate-ingest question. v2 same day: incorporates the owner rulings on success-ack grace (12h), retention (5 days), the attempt ceiling (4, composed not flat), no-silent-permanent-bans, the two hold kinds, and the F-NEW-MX boundary; incorporates the Rung-6 call-site audit, re-verified against the live codebase (see §F.8 for two facts the audit did not carry). v3 (2026-07-30): incorporates the five owner rulings from the rung-3 (F-NEW-NE) pre-implementation review — Bedrock ceiling enforcement timing, dispatch-attempts as a separate pre-epoch gate, watchdog counters as the inner retry tier, epoch ownership at the job DO, and the recovery_pending epoch-draw signal; amended same day with the rung-3 implementation review's ruling (§B) that an extract-arm epoch stop — signature stop or ceiling exhaustion — fails the whole job, converted by rung 3b into a retryable/held state like the other stop classes. v4 (2026-08-04): incorporates the L6 pre-implementation review and its owner rulings — new §B3 "the hold releaser" (every hold kind must name its releaser at the site that creates it, per the L5 device pass's `blocked(vendor_unavailable)` gap); the post-L5 breaker contract (§2, dispatch-admission-only consultation); the §C rollout-order precondition marked satisfied by L5 with a pre-launch caveat, plus the admission-record storage posture and the success-adoption window's owner-accepted unbounded-age basis; the §D `/dispatch` refusal contract; and F-NEW-MX's schedule moved to its own sprint immediately after L6. v5 (2026-08-06): L7 (F-NEW-NI) Phase 1 pre-implementation rulings, incorporated as dated amendments in §4 rather than rewritten prose — granularity ruling 5 superseded (full per-step detail in both environments, not staging-only); alerting brought into L7 scope (superseding "it does not do alerting/paging"); F-NEW-NL ruled (meta.rev dropped from L7's inputs, classification by applyMeta label instead); and the spend-rollup sentence corrected (`ingest_jobs` cost columns stay DO-computed, `ingest_spend_events` is additive audit trail only). v6 (2026-08-15): minimal §A annotations — the unacked-success arm's flat 5-day retention is superseded by INGEST_DELIVERY_DESIGN.md §9B ruling 3 / §9C (evidence-gated, 15-day outer cap, rung 5); the arm's "next poll's 410" premise corrected to `404 unknown_or_expired_job`. v7 (2026-08-26): L7 (F-NEW-NI) shipped in full — recordhealth-api `5dce9ff..698f26a` — incorporated as dated amendments rather than rewritten prose: §4 (alerting's final shape closing leg (c), the fifth NCC panel, the no-event-table-pruning ruling, the F-NEW-PO verdict lens), §A (the remaining retention arms), and §B2 (the balance-blocked never-dispatched ruling, F-NEW-PW). Mechanics for all of it live in INGEST_ARCHITECTURE.md and api `src/ingest-verdict.mjs`, not restated here.
...
A ceiling breach is not a silent terminal: it places the job on **hold** (`spend_cap` — ERROR kind at every site, owner ruling 2026-07-31; §B2) with an NCC notice — policy said stop, and a human sees that it did. Because the check happens at intent time and intents are fenced, unbounded spend requires either a ledger bypass or a vendor billing us for calls we never made. Exact numeric values for both ceilings remain an open ruling; what's locked is that LlamaCloud enforces from the start and Bedrock does not until rung-1 data exists.

**Rung-3 ceiling scope (ruling 1, rung-3 pre-implementation review, 2026-07-30):** rung 3's "ceilings" work is the epoch ceiling (§B) plus the already-enforcing LlamaCloud gate above — rung 3 does **not** flip `gateBedrock` to enforcing. Bedrock spend enforcement lands separately, at F-NEW-MX: hard, not soft, because refusal requires the hold machinery (§B2, built in 3b) and because a real ceiling number requires ledger data that only accumulates once rung 1's placeholder has been running in production. **Scheduling, updated (owner ruling 2026-08-04):** F-NEW-MX is its own sprint, immediately after L6 — superseding the earlier "hard-triggers once rung 3b ships" timing, and explicitly not gated on L7's admission-record cleanup (§C). The ceiling number itself is sourced from `ingest_jobs`' actual token-vs-pages distribution once that data exists, not an arbitrary placeholder value. (The ROADMAP F-NEW-NE scope line will be corrected to match, in a separate commit in the app repo.)

**Circuit breaker per vendor boundary — post-L5 contract (owner ruling 2026-08-04).** Breaker state stays cross-job and cross-user in one VendorHealthDO per vendor, but *where* it is consulted moved once L5 shipped:

- **Consulted at coordinator dispatch admission, only.** Before `dispatchOne`, alongside the OD-1 balance gate. Open → the entry parks as the existing Site A `blocked(vendor_unavailable)` shape (§B's dispatch-time amendment) — no DO hop, no vendor call. This replaces the earlier per-submit DO hop with a coordinator-local check.
- **The job DO reports outcomes; it does not consult the breaker.** At its existing vendor-error capture points, the job DO reports classified submit outcomes to the VendorHealthDO, best-effort — this feeds the trip/probe state machine without putting a breaker check on the mid-flight submit path.
- **The registry lives on the VendorHealthDO.** It owns the single half-open probe slot and a registry of coordinators with a park outstanding on this breaker — registered at park, cleared at release. Probe success pokes every registered coordinator's `reevaluateHeld` (§B3), not a blind broadcast.
- **Trip and backoff, unchanged:** 3 consecutive deterministic-vendor failures (402, 401/403) trips immediately; 5 transient failures within 5 minutes trips for transients; half-open admits one real job on a backoff schedule (60s doubling to 15 min).
- **Mid-flight submit points report but never consult, and never park (owner ruling 2026-08-04, v1 scope).** `submitChainExtract` and the 6b fallback upload report their outcomes to the VendorHealthDO like any other classified submit, but a mid-flight vendor failure takes the normal failure-class path (§B's table), not a breaker park — parking a job that already has forward progress would orphan work the marker/epoch machinery exists to protect. v1 consults the breaker at dispatch admission only; extending consultation to mid-flight submit points is out of scope here.
- The user-facing meaning is locked (ruling 4): "Processing delayed due to server issues. We will resolve and process your records shortly. No action is needed on your part." Exact wording is deferred to F-NEW-MV — design against the meaning, not the string.
...
> **Amendment (owner ruling, 2026-08-06) — alerting is now IN L7 scope, superseding the sentence above.** The panels remain (a hold notice is still a durable row that stays visible until explicitly resolved), but observability is no longer look-when-you-check-only. Detection has three legs: (a) event-time alerts fired from the hold/terminal write hooks — the same sites that write §B2 hold rows and §B terminal transitions; (b) a DO alarm self-check while work is live, no Neon touch (the alarm loop already runs; this adds a liveness self-check to it); (c) an infrequent dead-DO catch cron, interval to be ruled once the Neon autosuspend window is confirmed. Delivery: ntfy push to the owner's phone. Hard rules, non-negotiable: alert payloads carry job id, phase, and hold kind/reason ONLY — never document content, never anything resembling PHI, same allowlist posture as the event tables themselves; and alerting **observes and never gates** — same NCC principle as `error_events`'s standing rule 3, restated for this surface. This closes the "Does not solve" list's alerting/paging exclusion below — see that section's amendment note.

> **Amendment (owner ruling, 2026-08-25) — L7 shipped in full; alerting's final shape closes leg (c) above.** Delivery is ntfy.sh on a paid account, with `NTFY_TOKEN` bearer auth on every publish (api `2c75c73`) — anonymous publishing from shared Workers egress hit ntfy.sh's rate limit in practice (429s observed 2026-08-23), which is why the token exists. Every job-scoped alert now carries a `Click` header that deep-links to the NCC per-job drill-down (api `4c6b43f`). Legs (a) and (b) shipped as designed (api `c72c738`). Leg (c) — above, "interval to be ruled once the Neon autosuspend window is confirmed" — is superseded rather than merely ruled: it is not a Neon-touching cron at all. It closes as a daily, production-only janitor (staging untouched) whose 24-hour candidate threshold is derived from the existing lease/backoff/delayed-watch/epoch constants, which pokes the coordinator's existing hold-reevaluation path or the job DO to re-run its own alarm — poke, never rule — and deletes nothing (api `6378332`). Five panels ship, not four: the four below, plus a fifth, the vocabulary misfire log (INGEST_VOCABULARY_DESIGN §6 / OR-15 — not one of L7's four and never folded into them) — api `698f26a`. Mechanics: INGEST_ARCHITECTURE.md § L7 alerting, § L7 daily ingest janitor, WORKER_ARCHITECTURE.md § The FIFTH NCC panel. Not restated here.
...
- `ingest_phase_events`: one row per phase transition (claimed, completed, expired, failed, superseded, held, released), carrying job_id, phase, gen, event, duration_ms, step_count, error_class, hold_kind, hold_reason, environment, t. Not per-step, not per-poll. Roughly 10 rows/job. "Which stage of which phase is slow" comes from duration_ms per phase plus a slowest_step column stamped at completion. Granularity is locked (ruling 5): staging carries full per-step detail permanently; production carries transitions only for successful routine work, plus full step detail on anything that fails.
  ...
  > **Amendment (owner ruling, 2026-08-06) — supersedes ruling 5 above.** Granularity is now full per-step detail in BOTH environments, success and failure alike — the staging-only/production-transitions-only split is dead. Rationale: the owner wants success-path step statistics for vendor selection and optimization, not just failure diagnostics; row volume is trivial at current scale; a retention sweeper can trim detail rows later if it ever matters. The per-step rows carry `event='step'` (added to the transition vocabulary above) and a `step_label` column (the `renewLease` step label), populated only on those rows. `waited_ms` is stamped on `dispatched` rows (dispatch time minus enqueued_at), consistent with the "Queue and concurrency" paragraph below.
  >
  > **Same ruling closes F-NEW-NL.** `meta.rev` is dropped from L7's inputs entirely — it plays no role in what gets emitted. Phase-event emission is classified by the `applyMeta` label at the call site: lease-renew labels emit `step` rows, transition labels emit transition rows. This resolves F-NEW-NL's open ruling ("keep rev as-is and filter at export, add a separate counter, or drop rev from L7's inputs") in favor of the third option.
  >
  > **Amendment (owner ruling, 2026-08-25) — supersedes the "a retention sweeper can trim detail rows later" remark above.** No event-table pruning, ever, at current scale. Size visibility substitutes for a cap: the daily janitor's size report over the growing NCC event tables, and the misfire panel's row/rollup counts, are the answer to "is this growing," not a sweep. Pointers: INGEST_ARCHITECTURE.md § L7 daily ingest janitor, WORKER_ARCHITECTURE.md § The FIFTH NCC panel.
...
- `ingest_spend_events`: the ledger flush, per attempt, per vendor, units + unit type + vendor job id. Flushed at terminal by the DO, and by the existing sweeper for jobs whose DO died before flushing. Spend is recorded in DO storage at intent time and exported at terminal-or-sweep. `ingest_jobs` cost columns become a rollup of ledger rows.
  ...
  > **Amendment (owner ruling, 2026-08-06) — corrects the last sentence above.** `ingest_jobs` cost columns REMAIN computed by the existing DO rollup, unchanged by L7. `ingest_spend_events` is a purely additive per-call audit trail with NO read dependency from that rollup — no view, no SQL re-derivation of totals against the exported rows. The table exists for auditability, not as a new source of truth for the rollup.
...
> **Amendment (owner ruling, 2026-08-24/25) — the read-time verdict lens (F-NEW-PO).** The panels above expose the state machine; they don't answer "how many packets actually succeeded." A pure, computed-never-stored roll-up (api `src/ingest-verdict.mjs`, commit `9eb7a60`) collapses the full state matrix into six verdicts — `succeeded`, `partial`, `failed`, `on_hold`, `in_flight`, `excluded` — with `in_flight`/`excluded` held outside the panel's arithmetic. Pointer only: INGEST_ARCHITECTURE.md § The ingest verdict module.
...
- **Unacked success (abandonment):** no `fetched` ever arrives (app deleted — iOS provides no uninstall signal). Retention cap: **5 days** from terminal (locked), then delete both artifact classes and mark the job `failed_final(retention_expired)` if unacked-failed, or leave complete with artifacts gone if unacked-success. Named consequence: a phone offline for more than 5 days after import re-uploads and re-pays on its next poll's 410 — accepted; the phone retains the source.
  ...
  > **Amendment (owner-authorized close-out, 2026-08-15) — supersedes the flat 5-day timer above.** See INGEST_DELIVERY_DESIGN.md §9B ruling 3 / §9C: retention on this arm is now evidence-gated (push-delivery evidence: `uninstalled` / `no_channel` / `reachable` / `undelivered`), with a 15-day outer cap for the `reachable`/`undelivered` classes, implemented as rung 5. Pointer only — see that doc for the mechanism, not restated here.
  >
  > **Correction — the "next poll's 410" premise.** No `410` ever existed for this case. A deleted job answers `404 unknown_or_expired_job`, the phone's existing branch (§D's `404` row, below). The named consequence (re-upload after the window) stands; only the wire code was wrong.
...
> **Amendment (owner ruling, 2026-08-25) — the remaining arms shipped.** Rung 5 (`dbf2fa3`) shipped only the evidence-gated unacked-success arm (see INGEST_DELIVERY_DESIGN.md §9C). This closes the other three: acked-success at fetched + 12h, gated on repair quiescence checked inside the delete claim's own OCC applier; the failure/hold window at 5 days from last state change, restarting on every state change; and the final-terminal window at 12h. `retention_expired` is a real terminal class per §B's table, built from the hold it closes so the epoch history, ledger rollup, and admission key survive it. `/cancel` was found on staging to leave canceled jobs with no `terminal_at` to anchor against; it now stamps and anchors on `canceled_at`. The balance-blocked never-dispatched gap this rung deliberately left open is ruled in §B2 below (F-NEW-PW). Mechanics: INGEST_ARCHITECTURE.md § The other three retention arms (api `17a4456` + `96cd953`).
...
| vendor_billing, vendor_auth | Server, breaker-mediated | Parks as `blocked(vendor_unavailable)` (error-hold-kind, §B2); the VendorHealthDO's half-open probe success pokes registered coordinators' `reevaluateHeld` (§B3), which re-dispatches released entries |
| document_fault | Server, version-gated | ERROR HOLD, no clock but bounded by §A's 5-day retention. Job stamps the pipeline/prompt/app-schema version at hold; a version-change release, evaluated inside L6's `reevaluateHeld` (§B3) — no cron, owner ruling 2026-08-04 — releases holds whose failure-version < current, once per version change. Automatic release does not reset the epoch count |
| Any retryable class | User, via the retry control | §D's endpoint; draws the same epochs, respects the same cooldowns |
| spend_cap / ledger ceiling breach | Gate or owner | ERROR HOLD (owner ruling 2026-07-31, uniform at all sites — a cap self-clears, superseding the original administrative placement). Released by a rate-card/cap change (F-NEW-MX/MY) or explicit owner action; owner release resets the epoch count |
...
**Dispatch-time (Site A) amendment (owner ruling 2026-07-31, shipped in rung 3b):** the table's mover/mechanism rows govern post-claim/mid-flight failures only. At dispatch, ALL classes hold rather than auto-retry — vendor_billing / spend_cap / vendor_fault / our_timeout → ERROR hold; vendor_auth → ADMINISTRATIVE hold (a revoked credential cannot self-heal). Code-level detail: INGEST_ARCHITECTURE.md § The hold model + the failed-state split.
...
3. **Lifetime is bounded by the recovery-epoch ceiling of 4 (locked).** A recovery epoch is any failure-driven re-entry into processing: a watchdog reclaim, an auto-retry, a breaker re-dispatch (marker-present only — a Site A park draws nothing on release, by construction; §B3, owner ruling 2026-08-04), a version-sweep release, a user retry poke that dispatches. The initial dispatch is free; forward-progress claims (first claim of each phase) are free; repair attempts within a COMPLETE_PARTIAL job are free (bounded by the memo and TOTAL_REPAIR_CAP — they are the cheap path and must not be rationed like re-ingests). The ceiling is only reachable by failures with *different* signatures — else bound 2 stopped it earlier — and when the 4th epoch fails, the job goes to ADMINISTRATIVE HOLD (owner investigation) with the full epoch history (class + signature per epoch) in the NCC notice. Four failures for four different reasons are therefore visible by construction: they are the only way to reach the ceiling, and reaching it produces a notice that lists all four.

**An extract-arm epoch stop fails the whole job (owner ruling, rung-3 implementation review, 2026-07-30).** Parse and extract are concurrent arms with independent lease generations (§1), but they are not independent with respect to what "the job succeeded" means: LlamaExtract is the primary source for the extract-covered sections (labs, demographics, providers, dates), the Atom Pass never covers them, and there is no fallback. When bound 2's signature stop or bound 3's ceiling exhaustion lands specifically on the extract arm, the failure is not absorbable as an ordinary partial result the way a narrative-section (Atom Pass) failure is — a record that ships without its structured clinical data is not a lesser-quality complete job, it is a job that failed to do the one thing only the extract arm can do. On an extract-arm stop the job's terminal is FAILED; assembly does not run past it into COMPLETE_PARTIAL. This is narrower than it sounds: ordinary extract failure below either stop threshold — a first failure, or repeats that haven't yet hit signature-memo's 2 or the epoch ceiling's 4 — is unchanged and stays tolerated, retried through the normal epoch machinery like any other failure class. Only the stop itself escalates, from "this arm is done trying" to "the job is done." Rung 3b, which converts every other stop class from a hard terminal into a retryable/held state (§B, §B2), does the same here: pre-3b this FAILED terminal is exactly that, a terminal; from 3b onward it joins the same hold surface as the other stop classes — ERROR HOLD for the signature stop (a deterministic pipeline pattern), ADMINISTRATIVE HOLD for epoch-ceiling exhaustion (owner investigation, per §B's table) — rather than being invisible or unretryable.
...
- Lands here: `document_fault` (version-gated), a job-level signature stop whose class is ours (`our_timeout` pattern, deterministic pipeline errors), anything version-gated; `spend_cap` / ledger ceiling breach (owner ruling 2026-07-31 — a cap self-clears, so it is error-kind at every site); `blocked(vendor_unavailable)` is classified error-hold-kind for retention and visibility, though its release stays automatic via the breaker probe.
- Release: version-change release (inside `reevaluateHeld`, §B3 — no cron, owner ruling 2026-08-04), breaker probe, or owner action. Automatic release does not reset the epoch count; owner release does.
...
- Lands here: token gate pending (F-NEW-MX, §G), subscription/balance blocks (the OD-1 balance gate's blocked entries), owner investigation (epoch-ceiling breach, duplicate-submission threshold). (`spend_cap` moved to the error kind, owner ruling 2026-07-31.)
...
|---|---|---|---|---|
| document_fault | ERROR | 5d → failed_final(retention_expired), tombstone survives | Yes, with the artifacts | Version change (reevaluateHeld, §B3); owner |
| Job-level signature stop (our classes) | ERROR | 5d → failed_final(retention_expired) | Yes | Version change (reevaluateHeld, §B3); owner |
...
| spend_cap / ledger ceiling | ERROR (owner ruling 2026-07-31, uniform at all sites) | 5d | Yes | Cap change; owner |
...
> **Amendment (owner ruling, 2026-08-25) — the balance-blocked, never-dispatched gap.** A Site A balance-blocked entry (parked before claim — no job DO meta, no forward progress) whose parked bytes expire under §A's clock re-enters via a fresh `/enqueue` as a FRESH job, not `needs_reupload`. `needs_reupload` presumes a hold record with a resumable identity to adopt against (§C) — that exists only for a hold reached *after* dispatch. A never-claimed Site A park has no such record to resume, so there is nothing for `needs_reupload` to point at. Wiring (the coordinator-side conversion and its NCC visibility) is deferred: F-NEW-PW. Pointer: recordhealth-api `17a4456`'s "Deliberately not wired" note.

---

## B3. The hold releaser

Every hold kind must name its releaser at the site that creates it; a hold whose releaser does not exist is not an edge case, it is a design bug. The L5 device pass proved exactly this: `blocked(vendor_unavailable)` shipped with a creation site (§2's breaker, dispatch admission) and no corresponding release path wired to it — entries parked and stayed parked. L6 closes that gap with release machinery, three parts.

**1. Coordinator `reevaluateHeld(q)`**, a sibling of `reevaluateBlocked`, run on poke and on a slow self-poll (`HELD_REEVAL_INTERVAL_MS`, armed by `nextAlarmAt` whenever a releasable hold exists — the same poke+backstop pattern the subscription gate already uses). For each releasable entry, in order:

1. **Artifact check, first, always.** If the held entry's source bytes are gone (`parkedMissing`), the hold converts to `payload_expired` — loud, terminal, and the entry is never dispatched. A release that dispatched against missing bytes would be exactly the bug this check exists to prevent.
2. **Reason-specific release test**, only once the artifact check has passed:
   - `vendor_unavailable`: the breaker is closed.
   - `document_fault`: the hold's stamped failure version is less than the running `PIPELINE_VERSION_STAMP`.
3. **On release:** `unblockEntry`, then the normal dispatch loop, with `release: true` stamped on the dispatch body so the job DO can distinguish a released re-dispatch from an ordinary first dispatch.

**2. Job DO `applyHoldRelease`**, an OCC applier routed through `applyMeta` — the hold-exit counterpart to `applyRetryReclaim`, and it re-arms using that same re-arm block. It accepts only `disposition === HELD`, and clears `job_disposition` / `hold` / `hold_kind` / `hold_reason`. Epoch accounting stays marker-driven (§B ruling 5): a Site A hold — parked before claim, with no forward progress to have lost — was stamped with no `recovery_pending` marker, so its release draws nothing, by construction, not by a special case written for this hold kind. A mid-flight hold's marker, if one was stamped when the hold was created, draws normally on the next claim. **Automatic releases never reset the epoch count** — only a human release does (§B, §B2).

**3. Expiry during a hold.** If a hold's artifact expires while the hold is still open, the coordinator's artifact check (part 1) converts the coordinator's own record to `payload_expired` first. Left there, the job DO's status view would go stale — still reporting the vendor hold that no longer describes reality. So the coordinator pokes the job DO over `/hold-update` (one applier, the same OCC discipline as every other write) so `/status` reports the honest `payload_expired`, not the stale vendor hold.

**Owner ruling 2026-08-04:** `/hold-update` ships with L6, not as optional plumbing — the coordinator and the job DO must never diverge on hold state by design. A coordinator-side conversion with no corresponding job-DO update is not a sanctioned intermediate state.
...
**The success-adoption window, restated precisely (owner ruling 2026-08-04):** the window in the table's third row above is `job_index` presence, full stop — unbounded age, until rung 5's sweeper stamps retention state and prunes it. The owner accepted this explicitly: unbounded age is fine because checksum match is the correctness bound (an adopted job's bytes are re-verified against the same content hash regardless of how old the index entry is), and `force-fresh` (below) is the escape hatch for the rare case where a stale adoption is actually unwanted. This is not a gap rung 5 needs to close before the window is safe — it is the accepted shape of the window until rung 5 adds retention-state pruning on top of it.

**The admission record — tombstones and the duplicate-submission counter, both visible (ruling 4).** The coordinator keeps one PHI-free admission record per `(content_hash, patientId)` key: enqueue count, first/last seen, tombstone (failure class + failure version, when present), hold pointer. It lives in `qstate.admission` — job-index posture: never-pruned, PHI-free, with GC deferred to rung 5 (owner-accepted 2026-08-04, conditional on L7 addressing the cleanup — an unpruned index is an accepted interim state, not a permanent one). Two rules hang on it:
...
**Rollout-order precondition — satisfied (2026-08-04).** The precondition above is met: the app-repo side of §D's retry endpoint shipped across L5 (app repo `fa17a53` through `781e784`), confirmed by the F-NEW-NR device pass on 2026-08-04. Pre-launch caveat: no old-app fleet exists yet — there are no installed clients running the pre-L5 `.freshSubmission` mechanism this paragraph exists to protect — so the old-app-compatibility behavior described above is moot until launch. It stays documented because it becomes live again the moment a released app predates L5 in the field; it is not moot forever, only moot now.
...
**Internal `/dispatch` refusal contract (owner ruling 2026-08-04).** The coordinator-to-job-DO `POST /dispatch` (§B's `dispatch_attempts` gate) is self-describing on every refusal — there is no bare failure shape. A refusal is either `{held: true, ...hold record}` (409) or `{final: true, reason_class}` (409); the coordinator reads either answer and syncs its own index through `applyTerminal`, the same sync path it uses for any other terminal/held answer. `dispatch_attempts` (capped at `MAX_DISPATCH_ATTEMPTS = 3`, §B) counts only shapeless mechanics failures — a throw, a 5xx, a non-JSON body — because those are the only refusals with no informative answer to sync against; a self-describing refusal is not a dispatch failure, it is dispatch succeeding at telling the truth, and it must not burn an attempt. Owner ruling 2026-08-04 confirms the 409 + `held` marker shape specifically.

**Shipped wire_state decision (owner-confirmed 2026-07-31, rung 3b):** held jobs present the top-level `state` as `"failed"` on the existing wire for stop-class holds and as `"running"` for dispatch-time (Site A) holds — the locked F-NEW-MV "processing delayed, no action needed" meaning; `hold_kind` / `hold_reason` / `attempt_epoch` ride alongside as additive fields. L5 builds against this. Code-level detail: INGEST_ARCHITECTURE.md § The hold model + the failed-state split.
...
- **F-NEW-MX** is its own sprint, scheduled immediately after L6 (owner ruling 2026-08-04 — supersedes the earlier "hard-triggers once rung 3b ships" timing, and is explicitly not gated on L7's admission-record cleanup, §C): it needs the hold machinery (§B2) for refusal and a real Bedrock ceiling number, sourced from `ingest_jobs`' token-vs-pages distribution once that data exists rather than an arbitrary placeholder. Its plug points (§G) exist from rung 3b and default to proceed until MX ships.

---

## Does not solve

Sunk vendor spend; Bedrock 429 quota policy; ~~alerting/paging (hold notices are durable NCC rows, not pages)~~ **[Amended 2026-08-06: alerting is now IN L7 scope — see §4's dated amendment. NCC rows-as-durable-notices still holds independently; what's superseded is "does not do alerting."]**; cross-user fairness; already-leaked shards; the token gate's policy itself (F-NEW-MX); near-duplicate document detection beyond exact content-hash equality.

---

## Contradictions requiring owner ruling

- None open. The R2 deletion-site four-vs-five contradiction was resolved 2026-07-31 per the audit finding: four code-initiated deletion paths; blocked-entry expiry is a passive detector of the R2 lifecycle rule's deletion, not a deleter. §0 Correction 2 carries the reconciled enumeration.
