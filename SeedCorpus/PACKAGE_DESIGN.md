...
- R11. PHI values travel inside the package to the ADI, and the token assignments travel with them as part of the package (Worker-authored, OR-18 i).
...
manifest
  ...
  user_subject           : optional, opaque Apple sign-in id, sealed as given (OR-18 g)
core
...
- Relationships: RELATIONSHIP_DESIGN.md §1-§2.
...
**Verdict vocabulary, fixed:** accepted, rejected, corrected, with a server-side allowlist at submit.

---

## 7. Bakeoff linkage

The graded corpus is a set of packages with reviewer amendments. Ground truth is the corrected package built at the lock (ADI_GRADING_DESIGN §6, GR-16/17), not a fold of the log performed at scoring time: no resolver folds anything. The scorer (VENDOR_ABSTRACTION §4.1) compares any candidate package with the same `source_hash` against that corrected core. The manifest's configuration tuple is the report key. The corpus can now say which vendor and prompt produced each graded core, closing the weakness §4.1 accepted for v1.
...
5. **Token identity across devices.** Token ids are Worker-minted (OR-18 a), and per-user types mix in the sign-in id (OR-18 b). A package imported under a different sign-in id carries tokens that account's own ingests would not produce; the phone has no derivation of its own to reconcile them with (OR-18 a). Not designed further here; flagged for sprint 9. Narrowed by OR-18g: for shared and per-user phi_types alike, the user's sign-in id travels with the package to the ADI, so the ADI computes the identical token every time — no reconciliation of ADI tokens at graded return.
...
| 6 | api ADI | **DONE 2026-09-05** (server side shipped 2026-09-03; R16 record-not-refuse 2026-09-04; closed from the phone 2026-09-05). New baseline migration (`document_packages` under `review_documents`, `package_id` NOT NULL on the four projection tables, `grading_submissions` and `review_phi_detections` dropped, the append-only trigger — the first trigger on either ADI database); upload accepts a package; core to R2 sealed by hash and re-digested on every read; projections by the schema's derivation rules with DERIVED row ids; amendments write route with batch validation, optimistic concurrency and the scores routes (ADI_GRADING §4/§6); package read route; boundary hygiene (96 MB body cap — arithmetic, `base64(50 MiB pdf cap)` is 69,905,068 so the original 64 MB refused a body the route itself accepted; null-strip exemption for the core, provenance from the manifest, term ids by vocabulary version, section shape unified); a PHI token gap recorded as a finding, never a refusal (R16, 2026-09-04) | **Close condition MET 2026-09-05, from the phone.** Two real iOS-built packages accepted on staging through the pointer-contract / prompt-v7 Worker: 79 atoms / 13 PHI (sorted record) and 78 atoms / 41 PHI (unsorted record), zero PHI findings on either (neither R16 flag fired); the phone's re-submit of a held package answered 200 `{idempotent: true}` (2026-09-04); a patient-less package accepted (OR-15). The rebuild proof stands from the probe run of 2026-09-03: one real staging ingest submitted (201, 98 atoms / 144 regions / 144 links / 53 sections), the package row deleted cascading all 386 projected rows to zero, re-submitted from what `GET /v1/admin/packages/:id` returned with every row byte-identical, derived ids included; first real F1: atom 0.7407. Detail: `recordhealth-api/docs/WORKER_ARCHITECTURE.md § Package receive and store` | Opus 5 |
| OR-18 | api app | **DONE 2026-09-24**, inserted between sprints 6 and 7. Tokens minted by the Worker, read by the phone, computed identically by the ADI (§11 OR-18, shipped-marks) | Device proof 2026-09-24, package `9853ea6f`: all 11 PHI atoms carried a token and an independent recompute matched 12/12; an ADI mark minted, an unmark retired, a re-mark revived the same uid; no duplicates. Detail: `recordhealth-api/docs/WORKER_ARCHITECTURE.md § PHI tokens (OR-18)` | Opus 5 |
| 7 | api ADI | **Paused at step 2 (`6800ede`) until R2 ships; resumed under `ADI_GRADING_DESIGN_v1.3.md` §10's build order (v1.2 §2/§5/§6 superseded there), steps 1-4 shipped 2026-09-15 through 2026-09-17; build order replaced 2026-09-18 by `ADI_GRADING_DESIGN_v1.4.md` §10.** Console per ADI_GRADING_DESIGN_v1.5.md §7 (surface) and §10 (build order); "Accept N shown" retired unbuilt; links are moved through the Section and parent dropdowns, not drawn. **Resumes next at step 3, after the Apple Health sprint (ROADMAP F-NEW-VL)**, OR-18 having been inserted ahead of it. | One real document graded end to end: reviewer progress shown throughout, a corrected package produced at lock | Opus 5 |
| R1a | app | Phone-side edge inventory, FHIR ingest included, findings folded into RELATIONSHIP_DESIGN.md v1.1 §8.1 | See RELATIONSHIP_DESIGN.md §12 | Opus 5 |
| R1b | api | Stable identity options (§10) prepared for ruling | See RELATIONSHIP_DESIGN.md §12 | Fable |
| R2 | api | **DONE 2026-09-06.** Registry declaration, generator emits `relationships`, `atom_edges` retires from the wire | See RELATIONSHIP_DESIGN.md §12 | Opus 5, Fable reviews the registry declaration first |
| R3 | app | Decode the `relationships` array; AtomEdge becomes a projection; patient relationship store created | See RELATIONSHIP_DESIGN.md §12 | Opus 5 |
| R4 | app | FHIR ingest writes relationship entries | See RELATIONSHIP_DESIGN.md §12 | Opus 5 |
| R5 | app | Care graph absorbed (RL-10) | See RELATIONSHIP_DESIGN.md §12 | Opus 5, Fable reviews the kind registration first |
| 8 | api ADI | Graded package export: amendments appended, core hash unchanged | Export, verify hash, diff the log | Sonnet 5 |
| 9 | app | Graded import: amendments accepted, replace per R5, newest-wins view with history (ADI tokens need no reconciliation, OR-18 g; cross-account token identity is §8.5) | Round trip lands on the phone with original and graded both visible | Opus 5 |
...
- OR-12 (grading surface): ruled 2026-09-03, seven rulings GR-1..GR-7 in `ADI_GRADING_DESIGN.md` v1.0: verdicts live only in the amendment log, real-time appends with no lock-in, `grading_submissions` dropped in the sprint 6 baseline, gradeable classes read from the schema, "Accept N shown" scoped to the view, scores computed on demand, `reviewed` flag marks done. Shipped 2026-09-03 for the storage, write route and scores half (sprint 6 baseline; the table is gone); the console half is sprint 7. Scores-on-demand retired 2026-09-17 (GR-27): the console shows reviewer progress, never scores, and scoring is truth-versus-candidate (§7; ADI_GRADING_DESIGN v1.4 §5).
- OR-13 (whole-package checksum): ruled 2026-09-03, R15: `package_hash` on every version, verified and recorded at every door, every version kept. Sprint 6 lands the ADI side (verify on receive, record per version); sprint 8 the ADI export; sprint 9 the phone import; the phone's archive export gains it in sprint 9's app work. Sprint 6's ADI side shipped 2026-09-03 (verify on receive, one ledger row per version); sprints 8 and 9 pending.
- OR-14 (sprint 6 audit rulings, 2026-09-03): term ids added to the published dictionary snapshot so §4's by-version resolution is real (dictionary version bump); a second published declaration group for package entities (manifest, tokens, amendment, discovery, table_row) with its own coverage test; atoms declared authorable; ADI rows stay a narrow catalog, the console reads the package for detail; tokens layer served to the console from the package at receive; DB row committed before R2 writes with row removal on write failure; the atom-wire version header retires with the package upload; check-hash answers per package with the matching phone change in the same sprint; no lock flag anywhere; `package_id` NOT NULL on every projection row (reviewer-create routes take one); one shared hash helper. Shipped 2026-09-03 (sprint 6).
...
- OR-18 (2026-09-22). Token minting (supersedes R16 / OR-16 and the "INGEST WORKER MINTS NOTHING" sacred rule):
  ...
  d. Shared across users: providerName, providerPhone, providerFax, providerAddress, facilityName, facilityAddress, staffName, providerEmail (owner ruling 2026-09-23), and the five date types. Every other phi_type is per-user, patientFax included (owner ruling 2026-09-23).
  ...
  g. The user's sign-in id travels with the package to the ADI from this sprint, so the ADI computes the identical token for every type. No ADI-namespace tokens, no reconciliation of ADI tokens at graded return. The id rides in the manifest as `user_subject`, optional, opaque, sealed as given, never case-folded (`package_version` 2, `SCHEMA_VERSION` 27); a per-user mark with no `user_subject` is refused whole (409 `phi_mint_subject_missing`).
...
- OR-18 shipped (built 2026-09-22 through 2026-09-24; device proof in §9). One line per piece:
  - Dictionary token attributes (c, d): api `d6cd828..3b10588`.
  - Worker mint (a, b, e, k–p, u–w): api `501bb4e..28c788a`.
  - Shared store and anonymous documents (s, t, x–ab): api `ba319ec`, `ae96168`.
  - Mint endpoint (f): api `f1ea052`, `9a1a5e8`.
  - Manifest `user_subject` (g): api `6f08eb9`; app `9707fad`.
  - Phone reads Worker tokens (a): app `61fcd52..9707fad`; Apple Health mint hardening `85f4c91..3020c65`.
  - ADI re-key (g, i): api `248c761`, `c5cb0c8`, `43904e9`.
  - Staging ADI wipe (h): api `d598971`.
  - Docs: api `e4338a9`; app `f38aa90..f20c62a`.
- Open: none.
