...
6. **`F-NEW-SW`.** **Resolved 2026-09-22** — `package_lock_phi_untokenized`, shipped: the lock refuses a PHI fact with no live token. GR-35 rules the mark: it mints an ADI token, logged like any other reviewer change; GR-40's edit rule is superseded by the shipped rule (PACKAGE_DESIGN OR-18 i): a value edit moves the fact to the token for the new value, an unmark retires the token's use, a re-mark revives the same uid. The discovery declaration still has no `is_phi` / `phi_type`.
...
4. **PHI.** In order inside the step: the red-everywhere fix (GR-35), then the token audit, then the token build, whole (GR-41). Close, as shipped 2026-09-22: on staging, a fact marked PHI is red in card stripe, page box, grouping and PHI-only filter at once; a marked fact carries an ADI-minted token in the log (`package-tokens.mjs`, `ADI_TOKEN_SECRET`); editing it afterward moves the fact to the token for its new value, an unmark retires the token's use and a re-mark revives the same uid (PACKAGE_DESIGN OR-18 i, superseding GR-40's edit-behind-the-token rule); the lock refuses a PHI fact with no live token (`package_lock_phi_untokenized`).
...
- OR-18 (PACKAGE_DESIGN) moves minting to the Worker; the ADI mint shipped 2026-09-22 will re-key to the shared secret.
