# LAYER2_LIVENESS_DESIGN.md — On-device context layer rebuild failure, heal ladder, and the phone error channel (v1.1)

Status: DRAFT v1.1 (shape, not spec; owner rulings 1–7 fixed, the five 2026-09-09 rulings closed into §4; no open rulings)
...
`load` never sets `current = nil` on a failure when a stored layer exists: it serves the stored layer **with `freshness = .notCurrent(...)`**, and a view that renders atoms from it renders **one warning line above them and keeps rendering every one of them**. Copy through one pure mapping, `Views/ContextLayerFreshnessPresentation.statusCopy(for:) -> (title, body, action?)`, the `IngestProgressPresentation` convention (calm, em dash, no vendor names, no document names), rendered identically by the Dashboard patient summary card, `PatientProfileView`, the record-detail margin-dot header and the emergency card — owner-ruled copy, 2026-09-09 (§4 #16):

| Freshness | Title | Second line | Action |
|---|---|---|---|
| `.notCurrent(.healing)` / `.notCurrent(.owed)` | Health summary couldn't be updated. | Retrying automatically. | — |
| `.notCurrent(.exhausted)` | Health summary couldn't be updated. | Tap to update | Update now |
| `.unavailable(.healing)` / `.unavailable(.owed)` | Health summary isn't ready yet | We'll keep trying automatically — no action needed. | — |
| `.unavailable(.exhausted)` | Health summary couldn't be built | Your records are safe. Tap to try again. | Update now |

The two `.notCurrent` rows are the ruled strings, quoted exactly. The two `.unavailable` rows are the v1.0 strings, kept under ruling 4(d) — they were not separately ruled, they say nothing about being out of date, and there is no stored summary behind them to warn above. Three divergences from `IngestProgressPresentation`'s house style are the ruling's and are left as ruled rather than normalized: the ruled title ends in a period where that file's titles never do; "Tap to update" ends without one where its bodies are stopped sentences; and the action reads "Update now" in sentence case where `IngestProgressPresentation.Action.label` ships Title Case ("Try Again", "Upgrade"). P2 spells the strings from this table and from nowhere else.

The attempt number is not shown (the ingest surfaces show none either). Ruling 3's guarantee is structural: the only way `freshness` reads `.fresh` is a stored layer with `isStale == false` and no open ledger entry, and the only writer of that pair is a `.built` outcome that closed the entry.
...
| P2 | app | **Ledger, ladder, sweep, freshness, ingest consolidation.** `ContextRebuildLedger` (+ `PatientDeletionService` line), `ContextRebuildLadder` (pure), `ContextLayerHealSweep` (+ the four lifecycle sites and the wake timer), `ActiveContextLayer.freshness`, `ContextLayerFreshnessPresentation`, the warning line on Dashboard / `PatientProfileView` / record detail / emergency card (which keeps every clinical row beneath it), "Update now"; the DEBUG fault lever; the ingest path loses the 5 s retry and `.storageFailed` with every arm (§3.D), `IngestCompletionGate`'s third precondition per §3.D. | `ContextRebuildLedgerTests` (round trip, remove-on-delete, floor breach named), `ContextRebuildLadderTests` (every rung boundary, refusal is not an attempt, demand admits and resets exhaustion, a Layer 1 write re-opens), `ContextLayerHealSweepTests` (single-flight, due selection, owed drain, timer arming), `ActiveContextLayerFreshnessTests` (`.fresh` only from a `.built` that closed the entry; never nil with a stored layer), `ContextLayerFreshnessPresentationTests` (every case has copy; no case says "fresh" while an entry is open; no string in the mapping contains "out of date"), `EmergencyCardViewModelTests` (+ every row renders at every freshness — the warning line is above the rows, never instead of them), `IngestCompletionGateTests` (edited precondition), `AttentionCauseTests` (case gone), `IngestRetryModeTests` (arm gone) |
...
Decided here and reversible by the owner without a redesign: the doc's name (§3.A), `route` carrying the device site rather than a `site` column (§3.I), `observed_at` as a column (§3.I), the heal sweep as its own type (§3.E), deleting `.storageFailed` rather than retaining it producerless (§3.D), and the one copy question the ruling did not reach — `.notCurrent(.owed)` sharing the failure title (`F-NEW-SL`, §7).
...
- `F-NEW-SL` — `.notCurrent(.owed)` shows the ruled failure title ("Health summary couldn't be updated.") although an `.owed(.r3Projection)` entry records no failure yet, only a rebuild that is owed. The 2026-09-09 copy ruling addressed the failed case and the ruling's "ONE warning line" is honoured by grouping; splitting `.owed` off would need the owed reason carried into `ContextLayerNotCurrentReason` and a fifth ruled string, which is the owner's to say. Filed rather than invented. Owner: this doc §3.E.
