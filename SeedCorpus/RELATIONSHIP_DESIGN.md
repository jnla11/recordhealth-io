...
- RL-2. One declared vocabulary, the registry (§2). A kind that is not in the registry does not exist. Emitters can only emit registered kinds; the census reads its closed list from the registry; the published schema is generated from it; the console builds its controls from the published schema; the phone decodes against the published schema, flags unknown kinds loudly and renders them as text; a test fails the build when any emitter produces an unregistered kind. (Corrected at R3-3: "refuses" was the wrong verb for a CONSUMER. A kind the phone has not heard of is a kind the registry gained since its last schema fetch — refusing it would blank a real relationship over a stale cache, so the consumer renders it under the default behaviour and says it did not recognise it. An EMITTER producing an unregistered kind is still a build failure; that half is unchanged.)
...
- RL-7. Endpoints need ids that mean the same thing in every package and at every rebuild. Ruled 2026-09-05 (R1b): the Worker uuid minted once per parse and persisted in the core is the section id, and sections are addressed by it everywhere (amendment targets, ADI projection attribute, patient-store entries with `package_id` beside it). The v1.0/v1.1 premise "minted per rebuild" was wrong; the id already survives rebuild, repair, the ADI round trip, and the archive because the core is immutable. A new parse mints new sections and is joined by `continues`, never by id match. Tables get Worker uuids by the same rule at first assembly (the parse-side id embedding the record id, and the extract-side counter, both retire); rows stay `table_id:row_index`. Detail §10.
...
- In the amendment log: authored entries as `op: add, target.entity: relationship`. The VALUE IS NOT THE ENTRY (corrected at R3, audit rulings 40/41): it is the Worker's declared `relationship_authored` shape — `{ kind, source, target, note? }` — and the rest of the entry comes from the envelope the log already carries. The entry's `id` is `target.id`; `author` and `authored_at` are the amendment's own; `status` is `asserted` by definition, because an authored add IS an assertion. There is no `status` member on the wire and no `confirmed_via`: a reader that needs to tell "a person confirmed this" from "the walker computed it" reads `author`. Withdrawal is `op: remove` on the entry id; verdicts on a pipeline entry target the entry id. Shape per `PACKAGE_DESIGN.md` §3 and the Worker's `relationship_authored` declaration.
- On the phone, patient level: the relationship store (§3), append-only, encrypted like every sidecar, holding user, keystone, system, and inferred entries whose kinds are declared patient-home. Append-only has ONE carve-out — sacred rule 7 carve-out #3: the walker's `same_clinical_event` partition is REPLACED WHOLE on each rebuild, ids reused by triple (RL-11), because a re-emission over a document that already has entries would compound rather than replace.
...
authoring        : [reviewer | user | walker]   who may assert one through a log or store write
...
display_behavior : group | compose | date_of | same_as | text | none   how a consumer renders it (R3-3)
priority         : integer | null       order among `date_of` kinds (lower wins); null on every other row
...
**`display_behavior` and `priority` (R3-3, ruled 2026-09-07).** The phone holds no kind list: it reads the registry and dispatches on the BEHAVIOUR. `group` draws a header row over its members (a lab panel); `compose` folds members into one rendered value (an address); `date_of` resolves the source's date in `priority` order; `same_as` says two things are one thing; `text` renders a line — the honest placeholder for the five patient-level kinds no design has yet declared a behaviour for; `none` means declared and INDEXED but rendered by no behaviour, which is the ruling for the four containment kinds (`member_of_section`, `section_in_section`, `row_in_table`, `table_in_section`) because the section and table stores already carry that fact and drawing it again from the relationship list would be a second carrier. An UNKNOWN kind renders as text — `none` is a declared silence, not the fallback. The three `date_of` priorities (collected 1, observed 2, reported 3) are the shipped walk order `bestDate` used before the registry owned it.

**The three date kinds are PACKAGE-home and `cardinality: many` (R3-1 and audit ruling 42, ruled 2026-09-07).** A walker date row joins two atom ids inside ONE document, and RL-3 already says where such a fact lives; `authoring: ["walker"]` is a third authoring role, and it does not make those kinds reviewer-authorable (the ADI's filter reads `reviewer`). `many` because a result with two collection dates on the page produces two rows: the walker writes every row it finds and the phone resolves on READ by `priority` then same-page proximity. Resolving at write time was a silent drop of a real observation.

Regenerated 2026-09-07 by `node recordhealth-api/scripts/generate-relationship-registry-table.mjs` from the shipped `RELATIONSHIPS` declaration (`recordhealth-api/src/result-schema.mjs`) — the doc's table and the code cannot disagree (§11).

| kind | source → target | card. | home | emitted_by | authoring | inferable | fhir | display | priority | note |
|---|---|---|---|---|---|---|---|---|---|---|
| belongs_to_panel | atom_extract → atom_extract | one | package | pipeline | reviewer | no | Observation.hasMember, inverse | group | null | A lab result's membership in its panel: source the labValue row atom, target the labPanel header atom (row 0 of the same minted table). The extract adapter asserts it; table_cell_ref stays the cell's ADDRESS, never a second carrier of this fact (hard question 6). (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| belongs_to_address | atom_extract → atom_extract | one | package | pipeline | reviewer | no | Patient.address, flatten | compose | null | A city / state / zip atom's membership in its street-address anchor. Absent anchor ⇒ no entry. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| member_of_section | atom_narrative, atom_extract → section | one | package | pipeline | reviewer | no | Composition.section.entry, inverse | none | null | An atom's owning section. Narrative atoms: from the Atom Pass `section_id` stamp. Extract atoms: from LINE CONTAINMENT at assembly — the innermost section whose containedLineRanges hold the atom's resolved (page, line) — stamped onto the atom's `section_id` so field and entry agree by construction; no containing section ⇒ no stamp, no entry (ruling I-4). The field stays on the atom as the carrier; a test pins field and entry agree (hard question 1). (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| section_in_section | section → section | one | package | pipeline | reviewer | no | Composition.section.section, inverse | none | null | A section's parent. Emitted at assembly from `parentId` (one entry per section with a non-null parent); the field stays as the section builder's carrier. Registered pipeline at R2 rather than sprint 10 because the discipline test requires an emitter for every pipeline row (ruling I-3). (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| row_in_table | table_row → table | one | package | null | reviewer | no | null | none | null | Sprint 10. Until then TableCellRef's composite key is the containment carrier; no emitter. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| table_in_section | table → section | one | package | null | reviewer | no | null | none | null | Sprint 10. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| collected_at | atom_narrative, atom_extract → atom_narrative, atom_extract | many | package | walker | walker | no | Observation.effectiveDateTime, same | date_of | 1 | Phone date edge, Layer 2: a result → its collection dateAtom. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| reported_at | atom_narrative, atom_extract → atom_narrative, atom_extract | many | package | walker | walker | no | DiagnosticReport.issued, same | date_of | 3 | Phone date edge: a result → its report dateAtom. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| observed_at | atom_narrative, atom_extract → atom_narrative, atom_extract | many | package | walker | walker | no | Observation.effectiveDateTime, same | date_of | 2 | Phone date edge: an observation → its observed dateAtom. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| same_clinical_event | atom_narrative, atom_extract → atom_narrative, atom_extract | many | patient | walker | user | yes | null | same_as | null | Phone, existing: two atoms belong to one clinical event. (proposed: confirm_at, consumer_policy, adi_visible) |
| member_of_event | document, interaction, section, atom_narrative, atom_extract → event | many | patient | null | user | yes | null | text | null | EVENTS_INTERACTIONS §A.2 EventMembership; granularity is the source entity. Confirmation is a keystone answer. (proposed: confirm_at, consumer_policy, adi_visible) |
| part_of_interaction | document → interaction | one | patient | null | user | yes | DocumentReference.context.encounter, same | text | null | The fragmented-report case (§4): a document belongs to a visit. (proposed: confirm_at, consumer_policy, adi_visible, fhir) |
| continues | document → document | one | patient | null | user | yes | DocumentReference.relatesTo, same | text | null | A report that extends another; ALSO the join between a re-parse's package and the prior package (§10) — never an id match. (proposed: confirm_at, consumer_policy, adi_visible) |
| has_role | person → document, interaction | many | patient | null | user | yes | PractitionerRole, same | text | null | §6: the role value rides the ENTRY (`role`: patient \| treating_provider \| ordering_provider \| performing_technician \| interpreting_provider \| facility), per document. adi_visible false is RULED (§6), not proposed. The FHIR element is per role (PractitionerRole / Observation.performer / ServiceRequest.requester) and resolved by the export table, not here. (proposed: confirm_at, consumer_policy, fhir) |
| affiliated_with | person → organization | many | patient | null | user | yes | PractitionerRole.organization, same | text | null | Absorbs ProviderEntity.organizationIds / OrgEntity.providerIds (§8.1, R5). (proposed: confirm_at, consumer_policy, adi_visible) |
| (care graph kinds) | per CareGraphEdgeKind | per kind | patient | walker (CareGraphBuilder) | none | yes (carries confidence today) | per kind | per kind | null | the 16 CareGraphEdgeKind cases register under their existing names at R5 (RL-10); rows generated then, not hand-listed here |

`inferable`, `confirm_at`, `consumer_policy`, and `adi_visible` per kind are proposed values pending the inference sprint's own design; the columns are ruled, the numbers are not. The published registry carries a `proposed` marker on those columns so no reader takes a proposed number as final (R2 ruling I-10).

**Not registered (R2 ruling I-1, 2026-09-05):** `SectionRef`'s five sub-kinds (requester, basedOn, performer, subject, encounter). The live Worker emits none; the only producer is a dead Section Pass v3 prompt; no consumer reads them; the phone type retires at R3. Registering kinds for a shape that is itself being retired is scaffolding. When a real emitter of section references exists, its kinds register then, under their own design.

**The entry shape is declared once (R2 ruling, 2026-09-05).** The Worker's published schema declares the full entry of §1, including the members the Worker never emits (`confirmed_via`, `confidence`, `evidence`, `inferred_by`, `supersedes`), marked optional and never-Worker-emitted and exempt from the emit-coverage walk. The phone decodes against that published shape and declares no shape of its own.

Registry discipline, mechanical, not advisory: the registry is the source the generator reads; the emitters import their kind constants from it; the census reads its closed list from it; a coverage test asserts every emitted kind is reached by the rich fixture and every registered `emitted_by: pipeline` kind is actually emitted; the console reads the published registry and holds no kind list (grep-pinned); the phone decodes the published registry and treats an unregistered kind as a loud FLAG rendered as text, never a silent drop (RL-2, corrected at R3-3). When the registry changes, `SCHEMA_VERSION` bumps, and **a package is read under the CURRENT registry.** (Corrected at R3, audit finding 2: v1.3 through v1.4 cited a PACKAGE_DESIGN OR-2 rule of the opposite shape — "every package is read under the registry it was minted against" — and PACKAGE_DESIGN contains no such rule. The citation was to text that does not exist and is deleted here and in the code comment that copied it.) All existing data is re-ingestable, so a package minted under an older registry costs a re-ingest, not a compatibility layer. A per-version registry is filed as **F-NEW-SA** with NO owner and no reason: it gets a number so the idea is not re-invented, and it stays unowned until a reason to read a package under a retired registry actually appears.
...
**Patient home.** The phone's relationship store holds every entry whose kind is `home: patient`: `same_clinical_event`, event membership, interaction and document joins, person roles, and every inferred entry. It is append-only, device-encrypted, part of the essential CloudKit set, and exported in the `.rhpkg` archive beside the packages (the archive already carries the patient index slice; the store joins it). It never travels to the ADI (`adi_visible: false` kinds) except where a kind explicitly allows it. Append-only has ONE carve-out here — sacred rule 7 carve-out #3: the walker's `same_clinical_event` partition is replaced whole per rebuild, ids reused by triple (RL-11). The carve-out is scoped to that partition and to nothing else in the file.

**The walker writes in BOTH homes, and RL-3 is what sorts its rows (R3-1, ruled 2026-09-07).** v1.4 read as if the walker were a patient-store writer by definition. It is not: the rule sorts by what the fact SPANS, not by who wrote it.

- `collected_at` / `reported_at` / `observed_at` join two atom ids INSIDE one document. They go in that document's package amendment log, as `op: add` entries authored `walker` — the phone's second writer at the package door after the reviewer. This is also the only answer that works for an UNSORTED record: it has one package and therefore one home, where a per-patient file would have had none.
- `same_clinical_event` joins atoms ACROSS documents. No single package holds both ends, so it stays in the patient file.

The walker's log write is decided by DIFF and nothing else: an identical triple set means no write at all; otherwise remove-and-add over the changed triples, one `amendment_version`, and the package is re-sealed. There is no walker version constant — the log's live rows ARE the record of what the walker last produced, and the diff against them is the only decision (the version-constant convention, §15). A live row whose triple is still produced keeps its id across every build (RL-11).
...
Author values and what they may do: `pipeline` asserts package-home kinds deterministically at emit; `walker` asserts deterministically on device, into BOTH homes (R3-1): the three date kinds into the document's own package amendment log, `same_clinical_event` into the patient store — §3 says which and why; `reviewer` asserts, withdraws, and judges package-home kinds through the amendment log; `user` asserts patient-home kinds by confirming a keystone or forward-assignment question, or by drawing one directly where the UI allows; `system` writes inferred entries and the reconciliation entries sprint 9 needs (token maps, imports). No author may write an asserted entry of a kind whose registry row does not list it.
...
A relationship is one gradeable class, `relationship`, with a per-kind breakdown of progress counts — vetted, corrected, and deleted per kind, for the reviewer's own progress, never a score — because `belongs_to_panel` measures the extractor and `member_of_section` measures the Atom Pass and one merged count hides which regressed. Kinds with `emitted_by: null` progress the same way: there is nothing to vet until the reviewer authors one, so the kind sits at zero until then.
...
- **AtomEdge is DELETED, not projected (R3-5, ruled 2026-09-07 — this replaces v1.4's "becomes a display projection").** A per-atom copy of a relationship IS a field on one participant, which is exactly what RL-1 forbids, and persisting it in `.context.enc` beside the entry gave one fact two places to drift. `AtomEdge`, `EdgeKind`, `EdgeSource`, `ContextAtom.atomEdges` and `SegmentedFact.atomEdges` all go. What replaces them is a `RelationshipIndex` computed at build and held TRANSIENTLY on the layer — never persisted — which the four readers walk. `bestDate` takes its order from the registry's `priority` and breaks a tie by same-page proximity (`SourceRef` carries no line index, so proximity is char-offset order on the page: codex reading order, not visual line count, which is the known limit of the rule). Layer 1 carries no relationships. A kind the registry does not declare renders as text and is flagged, per RL-2.
...
| `SectionRef` (`references[]` on the parent, open string kinds, zero consumers, no live emitter) | section → section typed references | retired, not absorbed (R2 ruling I-1): nothing emits it, nothing reads it; the phone type goes at R3 with no registered replacement | R3 |
...
## 10. Stable identity (RL-7, ruled 2026-09-05)

**What the R1b audit found.** Section ids are minted once per parse (`buildSectionsFromCodex`, one caller, `processParse`) and written to the parse-generation sections shard; assembly, repair, retry adoption, the phone's decode and rebuild, the ADI projection, and the archive all reuse that id. Repair selects failed sections by id and republishes the same array with new inventories. The only event that mints new section ids is a new parse, which also regroups lines (F-NEW-RH), so no scheme survives it. The real gaps were two: section verdicts and the ADI projection addressed sections by array position (ordinal) instead of the uuid that already existed, with no endpoint check in the write route; and table ids were wrong (parse-side embeds the phone's record id, a cross-package identity inside a package-scoped object; extract-side is a bare counter that recurs in every package).

**Ruling.** The persisted Worker uuid is the section id. Inside a package a target is `{ entity: section, id: <uuid> }`. Outside a package (patient store, FHIR export) the id travels with `package_id` beside it. The ADI projection keeps `(package_id, ordinal)` as its row key and carries the uuid as the addressable attribute. A re-parse is a new package with new sections; the prior package is retained and the join is a `continues` relationship. Tables: Worker-minted uuid at first assembly, same shard discipline, both prior id forms retire. Rows: `table_id:row_index`, unchanged. Cells: `table_cell_ref`, unchanged.

**Provenance.** The id is minted at the original capture and never regenerated; it lives inside the core, which is stored byte-exact and sealed by `core_hash` at every door. Its stability is a consequence of the core's immutability, not a promise the code keeps. The id is a label; proof is the bytes and the hash.

**Rejected.** Content-derived hash: reorders the minting pass to buy recomputability that no reader uses, and breaks in the same case the uuid does. `package_id:ordinal`: gives one thing two identities, and a position is lost the moment a reader filters or reorders.

**Ships in R2:** amendment-target derivation for sections changes from ordinal to id; table ids become uuids; `SCHEMA_VERSION` bump; the write route verifies a section or table endpoint against the core when a batch carries one; three stale comments (`result-schema.mjs` section declaration, `package-projection.mjs` projectSections, PACKAGE_DESIGN §4 "minted per rebuild") corrected; ADI_GRADING §3's section line changes to id.
...
| R1b | api | **DONE 2026-09-05.** Identity audit; §10 ruled (persisted Worker uuid, option 3) | Done | Fable |
| R2 | api | **DONE 2026-09-06** (Fable spec pass + eight implement steps). Registry declaration, generator publishes `relationships` and its vocabularies, `SCHEMA_VERSION` 12 → 13, the top-level `relationships` array replaces `atom_edges` (extract-side entries persisted per generation so ids mint once; `member_of_section` and `section_in_section` built at assembly, extract atoms by line containment), table ids become uuids, the empty `section_inventory.edge_kinds` census replaced by `timing.relationships_by_kind`, census and misfire read the registry, amendment validator gains the `relationship` class with endpoint verification against the core and named refusals, scores gain `by_kind` and `authored_only`, write and scores routes read the core on demand, `normalizeAmendmentIds` extended, baseline_007 drops `data_atoms.atom_edges`, the three reviewer-create routes and the links DELETE deleted, dictionary `rh.edge.*` terms retired by the owner (dictionary v3) | **Close condition MET 2026-09-06.** Staging deploy `f337320e-5d28-409f-8918-534142559628`, production `bc7b9b34-bf3f-4a39-89dc-b45a714de78a`; `baseline_007_adi_relationships.sql` + `vocabulary_r2_retire_edge_terms` applied and ledgered on both ADI databases 2026-09-06, dictionary v3 both envs; live proof job `da4b0f54` / package `8d531df7` on staging, 192 served relationship entries (72 belongs_to_panel / 3 belongs_to_address / 109 member_of_section / 8 section_in_section), no `atom_edges` on the wire; `GET /v1/schema` served version 13; an authored `belongs_to_panel` add accepted, the cardinality case refused; all probes as expected, suite green | Opus 5 |
| R3 | api + app | **Step S (api) DONE 2026-09-07.** Registry gains `display_behavior` + `priority` (R3-3) and `display_behaviors` joins the published vocabularies; the three date kinds move to `home: package`, `authoring: ["walker"]`, `cardinality: many` (R3-1, ruling 42); `RELATIONSHIPS_SINCE` decoupled from `SCHEMA_VERSION`; the atom `producer` becomes a ROLE (`extract` / `narrative`) with `EXTRACT_CONFIG_VERSION` x2; the configuration tuple gains `inference_provider_by_pass`; `AMENDMENT_AUTHORS` gains `"walker"` and a relationship `add` is scored BY AUTHOR (a walker add never bumps `discoveries`); the extract adoption gate reports `adopted_from`; the version-constant convention's pins land (§15); `SCHEMA_VERSION` 13 → 14. **Steps 1–9 (app):** decode the `relationships` array, entry ids persisted never minted (RL-11); AtomEdge DELETED and the index transient (§8); the registry read and the display dispatch; walker date rows into the package log and `same_clinical_event` in the patient file (§3); `SectionRef` retired; `bestDate` walks the index by registry priority then proximity | Ingest on staging renders panels and addresses grouped from the list, by BEHAVIOUR and naming no kind; wipe, rebuild, identical by entry id | Opus 5 |
...
- v1.3 (2026-09-05): R2 spec rulings folded. I-1 SectionRef sub-kinds NOT registered (scaffolding); I-3 `section_in_section` emitted at R2; I-4 extract atoms get pipeline `member_of_section` by line containment (owner flipped the spec's hand-authoring choice); I-8 `edge_kinds` census removed; I-9 dictionary terms retired by the owner after deploy; I-10 `proposed` marker published; I-11 four-entry fold confirmed; the entry shape declared once in the Worker schema with never-emitted members optional (owner ruling over the spec's split declaration). The remaining spec choices (references prefix moot, scores route reads the core on demand, atom PATCH waits for console step 3, adoption gated on shard presence, pre-R2 repair defaults to an empty extract list) accepted as taken.
- v1.2 (2026-09-05): §10 ruled after R1b (RL-7 rewritten; the "minted per rebuild" premise retracted); R2 scope gains the section-target derivation, table uuids, and the endpoint check.
- v1.5 (2026-09-07): R3's owner rulings R3-1..R3-11 and the 2026-09-07 audit's rulings 40/41/42 folded, and R3's server step shipped. R3-1 the walker writes in both homes, sorted by RL-3 (§3, §5); R3-3 `display_behavior` + `priority`, six values including `none` (§2), and RL-2's consumer verb corrected from "refuses" to "flags and renders as text"; R3-4 `producer` is a ROLE; R3-5 AtomEdge deleted rather than projected (§8, replacing v1.4); rulings 40/41 the authored value is `relationship_authored`, not the entry (§1); ruling 42 `cardinality: many` on the date kinds with read-time resolution; finding 2 the PACKAGE_DESIGN OR-2 citation in §2 deleted as a citation to text that does not exist, with the current-registry rule stated and F-NEW-SA filed unowned; finding 6 PACKAGE_DESIGN §3's "derived layers are never in the package" narrowed to INFERENCES. Point-in-time spec: `RecordHealth_App/docs/archive/R3_RELATIONSHIPS_SPEC.md`.
- **The version-constant convention (owner, 2026-09-07), verbatim:** *"No version constant a human has to remember to change. Every versioned artifact is either content-derived or guarded by a test that fails when its content changes without it."* Landed in `recordhealth-api` at R3 step S (`test/pipeline-config-pins.test.mjs`: one SHA-256 pin per versioned artifact, filed under the constant's value, failing in both directions) and recorded in both repos' CLAUDE.md. **This SUPERSEDES `VENDOR_ABSTRACTION_DESIGN.md` §1.1's "the bundle gets an explicit version constant (same convention as the existing prompt versions — not a derived hash)"** to the extent that a constant may stay explicit only under a guarding test. VENDOR_ABSTRACTION carries its own v1.2 update in F-NEW-RY's sprint; this entry is the pointer, and that doc is not edited here.
- Open: §2 per-kind `inferable`, `confirm_at`, `consumer_policy`, `adi_visible` values (rule with the inference engine design). Hard question 1 (`member_of_section` explicit or field-carried) and hard question 6 (panel edge versus cell ref), both ruled at R2's audit.
