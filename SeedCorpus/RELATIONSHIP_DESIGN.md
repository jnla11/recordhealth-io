...
Status: design v1.4 (shape, not spec), owner rulings applied, R2 spec rulings folded, R2 shipped; R3 next
Date: 2026-09-06 (v1.0 2026-09-05; v1.1 after the phone-side inventory R1a; v1.2 after the identity audit R1b; v1.3 after the R2 spec pass; v1.4 after R2 shipped, §2's table regenerated and §12 marked done)
...
Regenerated 2026-09-06 by `node recordhealth-api/scripts/generate-relationship-registry-table.mjs` from the shipped `RELATIONSHIPS` declaration (`recordhealth-api/src/result-schema.mjs`) — the doc's table and the code cannot disagree (§11).

| kind | source → target | card. | home | emitted_by | authoring | inferable | fhir | note |
|---|---|---|---|---|---|---|---|---|
| belongs_to_panel | atom_extract → atom_extract | one | package | pipeline | reviewer | no | Observation.hasMember, inverse | A lab result's membership in its panel: source the labValue row atom, target the labPanel header atom (row 0 of the same minted table). The extract adapter asserts it; table_cell_ref stays the cell's ADDRESS, never a second carrier of this fact (hard question 6). (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| belongs_to_address | atom_extract → atom_extract | one | package | pipeline | reviewer | no | Patient.address, flatten | A city / state / zip atom's membership in its street-address anchor. Absent anchor ⇒ no entry. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| member_of_section | atom_narrative, atom_extract → section | one | package | pipeline | reviewer | no | Composition.section.entry, inverse | An atom's owning section. Narrative atoms: from the Atom Pass `section_id` stamp. Extract atoms: from LINE CONTAINMENT at assembly — the innermost section whose containedLineRanges hold the atom's resolved (page, line) — stamped onto the atom's `section_id` so field and entry agree by construction; no containing section ⇒ no stamp, no entry (ruling I-4). The field stays on the atom as the carrier; a test pins field and entry agree (hard question 1). (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| section_in_section | section → section | one | package | pipeline | reviewer | no | Composition.section.section, inverse | A section's parent. Emitted at assembly from `parentId` (one entry per section with a non-null parent); the field stays as the section builder's carrier. Registered pipeline at R2 rather than sprint 10 because the discipline test requires an emitter for every pipeline row (ruling I-3). (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| row_in_table | table_row → table | one | package | null | reviewer | no | null | Sprint 10. Until then TableCellRef's composite key is the containment carrier; no emitter. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| table_in_section | table → section | one | package | null | reviewer | no | null | Sprint 10. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| collected_at | atom_narrative, atom_extract → atom_narrative, atom_extract | one | patient | walker | none | no | Observation.effectiveDateTime, same | Phone date edge, Layer 2: a result → its collection dateAtom. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| reported_at | atom_narrative, atom_extract → atom_narrative, atom_extract | one | patient | walker | none | no | DiagnosticReport.issued, same | Phone date edge: a result → its report dateAtom. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| observed_at | atom_narrative, atom_extract → atom_narrative, atom_extract | one | patient | walker | none | no | Observation.effectiveDateTime, same | Phone date edge: an observation → its observed dateAtom. (proposed: inferable, confirm_at, consumer_policy, adi_visible) |
| same_clinical_event | atom_narrative, atom_extract → atom_narrative, atom_extract | many | patient | walker | user | yes | null | Phone, existing: two atoms belong to one clinical event. (proposed: confirm_at, consumer_policy, adi_visible) |
| member_of_event | document, interaction, section, atom_narrative, atom_extract → event | many | patient | null | user | yes | null | EVENTS_INTERACTIONS §A.2 EventMembership; granularity is the source entity. Confirmation is a keystone answer. (proposed: confirm_at, consumer_policy, adi_visible) |
| part_of_interaction | document → interaction | one | patient | null | user | yes | DocumentReference.context.encounter, same | The fragmented-report case (§4): a document belongs to a visit. (proposed: confirm_at, consumer_policy, adi_visible, fhir) |
| continues | document → document | one | patient | null | user | yes | DocumentReference.relatesTo, same | A report that extends another; ALSO the join between a re-parse's package and the prior package (§10) — never an id match. (proposed: confirm_at, consumer_policy, adi_visible) |
| has_role | person → document, interaction | many | patient | null | user | yes | PractitionerRole, same | §6: the role value rides the ENTRY (`role`: patient \| treating_provider \| ordering_provider \| performing_technician \| interpreting_provider \| facility), per document. adi_visible false is RULED (§6), not proposed. The FHIR element is per role (PractitionerRole / Observation.performer / ServiceRequest.requester) and resolved by the export table, not here. (proposed: confirm_at, consumer_policy, fhir) |
| affiliated_with | person → organization | many | patient | null | user | yes | PractitionerRole.organization, same | Absorbs ProviderEntity.organizationIds / OrgEntity.providerIds (§8.1, R5). (proposed: confirm_at, consumer_policy, adi_visible) |
